...
public class M_DoorLockController extends SuperController implements Initializable {
    ...
    private void removeKeysFromInventory() {
        Object[] items = Arrays.stream(InventoryItems.values()).filter(m -> m.name().contains("KEY")).toArray();
        Arrays.stream(items).forEach(object -> {
            InventoryItems item = (InventoryItems)object;
            if(player.getHasInventoryItemBooleans()[item.ordinal()][0]) {
                    inventoryBuilder.removeItem(item);
            }
